...
我们将引入后缀为 `-rancherentX-X-X` RKE K8s 版本号作为 GC-KDM 中 Rancher Prime 使用的 k8s 版本号，该版本号符合 semver versioning 定义，并能很好的与开源版进行兼容。
...
1. 1.24.9-rancher1-2
1. 1.24.9-rancher2-2
1. 1.24.9-rancherent1-1-1
1. 1.24.9-rancherent1-1-2
1. 1.24.9-rancherent2-1-1
1. 1.24.10-rancher1-1

在相同的 k8s 小版本的情况下，semver compare 场景 `-rancherentX-X-X` > `-rancherX-X`，这样能直接适配开源 data.json 中 `Addons template` 以及 `Service Options` 选项，减少维护工作量。  
另外，在 `rancher/rancher` 中，使用了 library `github.com/mcuadros/go-version` ，该库已在 2020 年停止更新，但导致了无法完全按照 semver compare 的方式进行，需要使用 3 位序号保证 ent 版本比开源版版本号更大。
...
- releases(array): 如 channels.yaml 中，releases 定义，并对列表中的结构定义进行了扩展。
  ...
  - referenceVersion(新增参数): 引用的 RKE K8s 版本，支持输入 `""`(empty string)，`none` 或 一个特定的 RKE K8s 版本。输入为 `none` 为一个新版本；输入为 `""`(empty string) 时，使用上一个版本参数；输入特定的 RKE K8s 版本时，引用该版本参数。影响参数 `serviceOptions`, `windowsServiceOptions`, `systemImages`
...
releases:
  - version: v1.20.15-rancherent1-1
